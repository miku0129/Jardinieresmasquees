...
export const getPostsFromFacebook = async () => {
  try {
    const res = await axios.get(
      ...
      }/feed?fields=id,permalink_url,created_time,message,full_picture,story&access_token=${
...
export const getEventsFromPosts = async () => {
  ...
  const responses = event_ids.map((event_id) =>
    axios.get(
      `https://graph.facebook.com/v17.0/${event_id}?fields=id,name,cover,place,description&access_token=${
        import.meta.env.VITE_FB_ACCESS_TOKEN
      }`
    )
  );
